...
public class OFSConnectorServiceSecWSSamplePWHandler implements CallbackHandler {

	// the username and password we expect incoming WS calls to use
	private String user = "SSOUSER";	// In this exmaple this values will be used as principal to pass to T24
	private String pwd = "anything";
	...
	public void handle (Callback[] callbacks) throws IOException, UnsupportedCallbackException {
		for (int i = 0; i < callbacks.length; i++) {
			if (callbacks[i] instanceof WSPasswordCallback) {
				...
				OFSConnectorServiceSecWSRequestContext ctx = new OFSConnectorServiceSecWSRequestContext(pc.getIdentifer());
				OFSConnectorServiceSecWSRequestContext.setRequestContext(ctx);
			} else {
